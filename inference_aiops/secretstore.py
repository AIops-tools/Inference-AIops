"""Encrypted secret storage for inference-aiops.

Secrets (Inference API keys) are NEVER written to disk in plaintext. They
live in ``~/.inference-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``INFERENCE_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "inference-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "INFERENCE_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``INFERENCE_<TARGET>_PASSWORD``; ``prefix='INFERENCE_'`` and
