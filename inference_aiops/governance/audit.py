"""Unified audit logging engine — all Inference skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.inference-aiops/audit.db``.
...
_log = logging.getLogger("inference-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring INFERENCE_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the INFERENCE_AIOPS_HOME refactor
