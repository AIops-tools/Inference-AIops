...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``INFERENCE_RUNAWAY_MAX`` times within ``INFERENCE_RUNAWAY_WINDOW_SEC`` trips a
...
from inference_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing Inference-specific; the decorator wraps it into a
