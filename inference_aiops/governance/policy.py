"""Policy engine — rule-based access control for Inference MCP tools.

Rules are loaded from ``~/.inference-aiops/rules.yaml`` with hot-reload on file change.
"""
...
_log = logging.getLogger("inference-aiops.policy")
...
class PolicyEngine:
    ...
    def check_allowed(
        ...
        *,
        ...
        if os.environ.get("INFERENCE_POLICY_DISABLED") == "1":
            ...
            if not self._rules and risk_level in ("high", "critical"):
                return TierDecision(
                    ...
                    reason=(
                        ...
                        "require a named approver (INFERENCE_AUDIT_APPROVED_BY) by "
