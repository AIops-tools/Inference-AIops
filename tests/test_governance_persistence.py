...
throwaway home (``INFERENCE_AIOPS_HOME``) and assert that the rows compliance
evidence is built from actually land on disk, and that a high-risk write runs
and is audited with no gate in its way (the skill authorizes nothing).
"""
...
from inference_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("INFERENCE_AUDIT_APPROVED_BY", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("INFERENCE_AUDIT_APPROVED_BY", "sre-alice")
...
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: scale_to_zero must capture
    the BEFORE replica count and persist the restoring undo on disk — no approver
    set, since the skill no longer gates the write."""
    ...
    result = gov.scale_to_zero(application="app1", deployment="dep1")
    assert result["priorState"]["numReplicas"] == 3
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "scale_replicas_up"
    assert json.loads(undo[0]["undo_params"])["num_replicas"] == 3  # captured prior state

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["scale_to_zero"]
