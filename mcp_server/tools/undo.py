...
the SAME governance path as any other call: audited and budget-checked, and
recorded under the inverse tool's own name and risk level. ``undo_apply`` is
itself governed; the inner tool it calls does the real work.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    own name and risk level. Pass dry_run=True to preview the inverse call
